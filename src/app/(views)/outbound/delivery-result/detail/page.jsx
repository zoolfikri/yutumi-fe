...
const DeliveryResultView = () => {
  const searchParams = useSearchParams();

  const id = searchParams.get("id");

  const [loading_get_delivery_note, setLoadingGetDeliveryResult] =
    useState(false);
  const [delivery_notes, setDeliveryResults] = useState([]);

  const user_data = useSelector((state) => state.user_data);

  const getDeliveryResult = useCallback(
    (id) => {
      setLoadingGetDeliveryResult(true);
      axios({
        method: "POST",
        ...
        (response) => {
          ...
          if (parseInt(code) === 200) {
            ...
            setDeliveryResults(temp_delivery_notes);
          } else {
            Sweetalert.fire({
              title: "Failed",
              text: message,
          ...
          setLoadingGetDeliveryResult(false);
        },
        (error) => {
          if (error.response) {
            const {
...
export default DeliveryResultView;
