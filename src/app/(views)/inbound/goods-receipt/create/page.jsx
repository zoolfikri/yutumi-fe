...
const PRCreate = () => {
  ...
  const postPurchaseRequest = (post_data) => {
    return axios({
      method: "POST",
      baseURL: process.env.NEXT_PUBLIC_API_URL,
  ...
  async function createPurchaseRequest(data) {
    ...
    try {
      ...
      const purchaseRequestResponse = await postPurchaseRequest({
      ...
      // Step 2: Save the purchase_request_id returned from postPurchaseRequest
      ...
      // Step 4: Send POST request to create item using postPRItems function
      const prItemsResponse = await postPRItems({ items: itemsPayload });
